...
def upload():
    print('\n','Uploading...', '\n')
    print('\n','Uploading path =',os.getcwd(),app.config['UPLOAD_FOLDER'],'\n')
...
def Stream():
    ...
    upDir = '/var/www/FlaskApp/FlaskApp/static/uploads/'
    for file in inFiles:
...
def download():
    # Compress file list
    base_path = pathlib.Path('/var/www/FlaskApp/FlaskApp/static/uploads/')
    data = io.BytesIO()
...
@app.route('/MapApp')
def MapApp():
    return render_template('MapApp.html')

if __name__ == '__main__':
    app.run(host="0.0.0.0", port=80) 
#ssl_context='adhoc', debug=True)
#ssl_context='adhoc',host="0.0.0.0",port=80)
